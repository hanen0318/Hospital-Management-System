...
namespace NLHospitalLibrary
{
	...
	public class Patients
	{
		...
		public Patients()
		{
			...
			oUpdCmd.Parameters.Add(new SqlParameter("@DateOfBirth", SqlDbType.NVarChar, 8, "DateOfBirth"));
		...
		public string DeleteData(string healthNum)
        {
			string msg = "";
            try
            {
				...
				thisDataSet.Tables["Patients"].PrimaryKey = keys;
				DataRow findRow = thisDataSet.Tables["Doctors"].Rows.Find(healthNum);

				if (findRow == null)
				{
					msg = " Patient with the Health Number " + healthNum + " not present in database.";
				}
				else
				{
					findRow.Delete();
					m_oDA.Update(thisDataSet, "Doctors");
					msg = " Patient with the Health Number " + healthNum + " deleted from database.";
				}
            }
            catch(Exception e)
            {
				msg = "Record was not deleted" + e.Message.ToString();
			}
            finally
            {
				m_oCn.Close();
